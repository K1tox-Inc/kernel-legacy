...
bool map_section(uintptr_t uspace_pd_phy, section_t *to_map)
{
    ...
    uintptr_t p_pool_addr = buddy_alloc_pages(to_map->mapping_size, HIGHMEM_ZONE);
    if (!p_pool_addr)
        return false;
...
{   
    ...
    section_t *last_sec = (data && data->data_buffer_size > 0) ? data : text;
