...
{
	if (unlikely(list_is_empty(&ready_queue)))
		return NULL;
...
{
	if (unlikely(!task) || list_node_is_linked(&task->sched_node))
		return -EINVAL;
...
void schedule(void)
{
	...
	if (unlikely(!idle_task))
		kpanic("Error: idle not init.");

	if (unlikely(cur_task == NULL))
		kpanic("Error: current task is NULL.");

	if (task_stack_overflow(cur_task))
		kpanic("Stack overflow: (pid=%d name=%s)", cur_task->pid, cur_task->name);

	if (cur_task->state == TASK_RUNNING && cur_task != idle_task && cur_task != dummy_task)
